...
class FFTModel2: ObservableObject {
    @Published var exponent: Float = 2.0
    ...
    func updateAmplitudes(_ fftFloats: [Float]) {
        ...
        let boosted = decibels.map { powf($0, self.exponent) }

        DispatchQueue.main.async {
            self.amplitudes = Array(boosted.prefix(self.numberOfBars))
        }
    }
...
public struct FFTView2: View {
    ...
    public var body: some View {
        VStack {
                ...
                if !placeMiddle {
                    ZStack {
                        ForEach(0 ..< barCount, id: \.self) { i in
                            if i < fft.amplitudes.count, let amplitude = fft.amplitudes[i] {
            ...
            // 👇 The exponent adjustment slider
            HStack {
                Text("Exponent: \(fft.exponent, specifier: "%.2f")")
                    .foregroundColor(.white)
                Slider(value: $fft.exponent, in: 0.5...5.0, step: 0.1)
            }
            .padding()
        }
        .onAppear {
            fft.numberOfBars = self.barCount
